		...
		<table>
			<tr>
				<template v-for="index in rangeArr(maxDepth)" :key="index">
					<th>test {{ index }}</th>
					<th v-if="index < maxDepth - 1" class="interCell"></th>
				</template>
			</tr>
			<tr
				v-for="indexR in rangeArr(4 * teamsCount + 2 * (teamsCount - 1))"
				:key="indexR"
			>
...
function matchColumnCellType(indexR: number, indexC: number): cellType {
	if (indexC === 0) {
		...
		else if (mod < 4) return cellType.empty
	...
	console.log(
		`indexR: ${indexR}, indexC: ${indexC}, countMatchesLeftTop: ${countMatchesLeftTop}, heightLeftTop: ${heightLeftTop}, countMatchesLeft: ${countMatchesLeft}, heightLeft: ${countMatchesLeftTop}`,
	)
