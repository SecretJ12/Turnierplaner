...
public class TestdataGenerator {
  ...
  private static final int NUMBEROFCOURTS = 4;
  ...
  public void generateData() {
    ...
    createTournament(
        TDate.BEFORE_REGISTRATION,
        "Clubmeisterschaft 2026",
        "Anmeldung ausstehend",
        true,
        compSetting);
    createTournament(
        TDate.REGISTRATION_OPEN, "Clubmeisterschaft 2025", "Anmeldung offen", true, compSetting);
    createTournament(
        TDate.BEFORE_GAMEPHASE, "Clubmeisterschaft 2024", "Anmeldung vorbei", true, compSetting);
    createTournament(
        TDate.GAMEPHASE_OPEN, "Clubmeisterschaft 2023", "Spielphase offen", true, compSetting);
    createTournament(
        TDate.AFTER_GAMEPHASE, "Clubmeisterschaft 2022", "Spielphase vorbei", true, compSetting);
    createTournament(
        TDate.BEFORE_REGISTRATION,
        "Clubmeisterschaft 2027",
        "Noch in der Planung",
        false,
        compSetting);
  }
  ...
  private Team[] createTeams(Competition competition, CompetitionSettings competitionSettings) {
    ...
    for (int i = 0; i < competitionSettings.getTeamNumbers(); i++) {
      ...
      if (competitionSettings.getCompetitionMode() == CompetitionMode.DOUBLE) {
        Player player2 =
            createPlayer(competitionSettings.getAgeRestr(), competitionSettings.getSex());
  ...
  private void addTeamsAndMatchesToGroup(
      Competition competition, CompetitionSettings compSettings) {
    ...
    for (byte k = 0; k < compSettings.getNumberOfGroups(); k++) {
      ...
      for (int i = k * compSettings.getNumberOfGroups() / compSettings.getNumberOfGroups();
          i < compSettings.getTeamNumbers() / compSettings.getNumberOfGroups() * (k + 1);
          i++) {
        for (int j = i + 1;
            j < compSettings.getTeamNumbers() / compSettings.getNumberOfGroups() * (k + 1);
            j++) {
    ...
    // TODO support more than 2 groups ? How is the tree supposed to look like again with Nextmatch?
    // @Jonas
  ...
  private void addTeamsAndMatchesToKnockout(
      Competition competition, CompetitionSettings compSettings, boolean finished) {
  ...
  /**
   * Create the competitions to a tournament. The method expects the tournament and a List of Tuple7
   * (Competition settings), the CompetitionType (Knockout, Group), CompetitionMode (Double,Single),
   * an integer (number of Teams) This should be a power of 2 for Knockout, a Boolean (individual or
   * joint sign up), number of Groups This should also be a power of 2, and name of Competition
   *
   * @param t the tournament to the competitions
   * @param compList A list of competition settings
   */
  private void addCompetition(Tournament t, TDate tDate, List<CompetitionSettings> compList) {
    for (CompetitionSettings compSetting : compList) {
      ...
      switch (tDate) {
        case BEFORE_REGISTRATION, REGISTRATION_OPEN, BEFORE_GAMEPHASE ->
      ...
      } else competition.setNumberSets(NumberSets.THREE);
      competitions.persist(competition);
      switch (tDate) {
        case BEFORE_REGISTRATION -> {}
