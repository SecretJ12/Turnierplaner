...
public class PlayerRepository implements PanacheRepository<Player> {
    ...
    public Player getByName(String first_name, String last_name) {
        return find("FROM Player p WHERE p.firstName = :firstName and p.lastName = :lastName", Parameters.with("firstName", first_name).and("lastName", last_name).map()).firstResultOptional().orElse(null);
    }
