...
public class PlayerRepository implements PanacheRepository<Player> {
  ...
  public Player getByName(String first_name, String last_name) {
    return find(
            ...
            Parameters.with("firstName", first_name).and("lastName", last_name))
        .firstResultOptional()
        .orElse(null);
  }

  public Stream<Player> filter(
      String search,
      Sex sex,
      LocalDate minAge,
      LocalDate maxAge,
  ...
  public Long countFilter(
      String search, Sex sex, LocalDate minAge, LocalDate maxAge, boolean admin) {
    return find(
            "#countFilter",
            Parameters.with("search", search)
                .and("admin", admin)
