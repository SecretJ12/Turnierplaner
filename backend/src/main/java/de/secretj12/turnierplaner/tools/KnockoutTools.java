...
public class KnockoutTools {
  ...
  public void updateKnockoutTree(Competition competition, jUserKnockoutMatch tree) {
    ...
    if (finale == null) finale = new Match();
  ...
  private int countTeams(jUserKnockoutMatch tree, int total) {
    int c = 0;
    if (tree.getPreviousA() != null) c += countTeams(tree.getPreviousA(), total - 1);
    if (tree.getPreviousB() != null) c += countTeams(tree.getPreviousB(), total - 1);

    if (tree.getPreviousA() == null) {
      if (total != 1) throw new BadRequestException("Invalid tree");
      if (tree.getTeamA() != null) c++;
      if (tree.getTeamB() != null) c++;
    }
    return c;
  }

  private Match updateKnockoutTree(
      Competition competition, jUserKnockoutMatch tree, Match match, int number) {
      ...
      if (match.getDependentOn() == null) {
        ...
        a =
            updateKnockoutTree(
                competition,
                tree.getPreviousA(),
                match.getDependentOn().getPreviousA(),
                number - 1);
        b =
            updateKnockoutTree(
                competition,
                tree.getPreviousB(),
                match.getDependentOn().getPreviousB(),
                number - 1);
      }
  ...
  private Team selectTeam(Match match) {
    ...
    if (match.getTeamA() == null && match.getTeamB() != null) return match.getTeamB();
    if (match.getTeamB() == null && match.getTeamA() != null) return match.getTeamA();
    return null;
  }
  ...
  private Match updateThirdPlace(Competition competition, Match finale, int total, int tCount) {
    ...
    if (tCount == 3) {
      if (thirdPlace != null) matches.delete(thirdPlace);
      return null;
    }
