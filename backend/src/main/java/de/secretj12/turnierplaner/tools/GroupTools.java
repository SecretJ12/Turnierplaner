...
public class GroupTools {
  ...
  @Inject GroupRepository groupsRepository;
  ...
  private void createGroups(
      Competition competition, List<Group> exGroups, List<Set<Team>> nGroups) {
    ...
    for (int index = 0; index < nGroups.size(); index++) {
      ...
      for (int i = 0; i < nGroup.size(); i++) {
        for (int j = i + 1; j < nGroup.size(); j++) {
          ...
          if (teams.contains(nGroupList.get(i)) && teams.contains(nGroupList.get(j))) continue;
  ...
  private void createKnockoutTree(Competition competition, List<Group> groups, Match thirdP) {
    if (groups.size() == 1) {
      ...
      if (thirdP != null) matchRepository.delete(thirdP);
      ...
      if (thirdP == null) thirdP = new Match();
      thirdP.setNumber(0);
      thirdP.setDependentOn(null);
      ...
      do {
        ...
        for (int i = 0; i < matches.size() / 2; i++) {
          Optional<NextMatch> exNext =
              matches.get(2 * i).getPreviousOfA() == null
                  ? Optional.empty()
                  : matches.get(2 * i).getPreviousOfA().stream()
                      .filter(NextMatch::isWinner)
                      .findFirst();
          if (exNext.isEmpty()) {
            Match m = new Match();
            m.setNumber(counter);
            ...
            matchRepository.persist(m);
            newMatches.add(m);
            if (matches.size() == 2) competition.setFinale(m);
            ...
            matchRepository.persist(m);
            newMatches.add(m);
            if (matches.size() == 2) competition.setFinale(exNext.get().getNextMatch());
          }
        }
  ...
  private Match finaleOfGroups(Competition competition, Group g1, Group g2) {
    Optional<FinalOfGroup> exFin =
        g1.getFinalOfGroupA() == null
            ? Optional.empty()
            : g1.getFinalOfGroupA().stream().filter(fog -> fog.getPos() == 1).findFirst();
    if (exFin.isEmpty()) {
  ...
  public List<jUserTeamGroupResult> determineGropuResults(Group group) {
    ...
    int rank = 1;
    Comp comp = new Comp();
    var sortedRes = results.values().stream().sorted(comp).toList();
  ...
  private static class Comp implements Comparator<jUserTeamGroupResult> {
    ...
    public int compare(jUserTeamGroupResult t1, jUserTeamGroupResult t2) {
      ...
      if (t2.getGamesWon() - t2.getMatchesLost() > t1.getMatchesWon() - t1.getMatchesLost())
        return -1;
      if (t1.getSetsWon() - t1.getSetsLost() > t2.getSetsWon() - t2.getSetsLost()) return 1;
      if (t2.getSetsWon() - t2.getSetsLost() > t1.getSetsWon() - t1.getSetsLost()) return -1;
      return Integer.compare(
          t1.getMatchesWon() - t1.getMatchesLost(), t2.getMatchesWon() - t2.getMatchesLost());
    }
  }
}
