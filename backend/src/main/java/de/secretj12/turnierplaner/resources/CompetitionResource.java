...
public class CompetitionResource {
  ...
  @Inject CommonHelpers common;
  @Inject PlayerResource playersResource;
  ...
  @Inject KnockoutTools knockoutTools;
  ...
  @Consumes(MediaType.APPLICATION_JSON)
  @Produces(MediaType.APPLICATION_JSON)
  public List<jDirectorCompetitionUpdate> getPrepareCompetitions(
      @PathParam("tourName") String tourName) {
    if (securityIdentity.hasRole("director"))
      return competitions.listByName(tourName).stream()
          .map(jDirectorCompetitionUpdate::new)
          .toList();
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public jUserCompetition getCompetition(
      @PathParam("tourName") String tourName, @PathParam("compName") String compName) {
    common.checkTournamentAccessibility(tourName);
  ...
  public String addCompetition(
      @PathParam("tourName") String tourName, jDirectorCompetitionAdd competition) {
  ...
  public String updateCompetition(
      @PathParam("tourName") String tourName, jDirectorCompetitionUpdate competition) {
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public List<jUserTeam> getSignedUpPlayers(
      @PathParam("tourName") String tourName, @PathParam("compName") String compName) {
    common.checkTournamentAccessibility(tourName);
    ...
    if (competition.getMode() == CompetitionMode.SINGLE
      ...
      if (regTeams != null
          && regTeams.stream()
              .anyMatch(
                  t ->
                      (t.getPlayerA() != null && t.getPlayerA().getId().equals(playerA.getId()))
                          || (t.getPlayerB() != null
      ...
      // double mode
      if (competition.getSignup() == CompetitionSignUp.INDIVIDUAL
          && competition.isPlayerBdifferent()) {
        ...
        if (reg.getPlayerA() != null) {
          ...
          if (regTeams != null
            ...
            throw new WebApplicationException(
                "Player already registered", Response.Status.CONFLICT);
        ...
        if (reg.getPlayerB() != null) {
          ...
          if (regTeams != null
              && regTeams.stream()
                  .anyMatch(
                      t ->
                          (t.getPlayerA() != null && t.getPlayerA().getId().equals(playerB.getId()))
                              || (t.getPlayerB() != null
  ...
  public String deleteTeam(
    ...
    if (dbTeam == null) throw new BadRequestException("Team does not exist");
    Competition competition = competitions.getByName(tourName, compName);
  ...
  public String updateTeams(
    ...
    for (var team : teams) {
      Player playerA =
          team.getPlayerA() == null ? null : players.findById(team.getPlayerA().getId());
      Player playerB =
          team.getPlayerB() == null ? null : players.findById(team.getPlayerB().getId());
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public jUserKnockoutSystem getKnockoutMatches(
      @PathParam("tourName") String tourName, @PathParam("compName") String compName) {
    common.checkTournamentAccessibility(tourName);
    ...
    if (competition.getType() != CompetitionType.KNOCKOUT)
      throw new WebApplicationException(
          "Competition does not have type knockout", Response.Status.METHOD_NOT_ALLOWED);
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public jUserGroupSystem getGroupMatches(
      @PathParam("tourName") String tourName, @PathParam("compName") String compName) {
    common.checkTournamentAccessibility(tourName);
  ...
  @Produces(MediaType.TEXT_PLAIN)
  public boolean initializeMatchesKnockout(
      @PathParam("tourName") String tourName,
      @PathParam("compName") String compName,
      jUserKnockoutMatch tree) {
    common.checkTournamentAccessibility(tourName);
  ...
  @Produces(MediaType.TEXT_PLAIN)
  public boolean initializeMatchesGroups(
      @PathParam("tourName") String tourName,
      @PathParam("compName") String compName,
      jDirectorGroupsDivision division) {
    ...
    // @formatter:off
    List<Set<Team>> groups =
        division.getGroups().stream()
            .map(
                group ->
                    group.stream()
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public jDirectorGroupsDivision groupsDivision(
      @PathParam("tourName") String tourName, @PathParam("compName") String compName) {
    common.checkTournamentAccessibility(tourName);

    Competition competition = competitions.getByName(tourName, compName);
    List<Group> groups = competition.getGroups();
    return new jDirectorGroupsDivision(
        groups.stream().map(g -> groupTools.teamsOfGroup(g)).toList());
  }
  ...
  public String updateMatches(
    ...
    for (var cMatch : matches) {
      ...
      if (match == null) throw new NotFoundException("Could not find match");
