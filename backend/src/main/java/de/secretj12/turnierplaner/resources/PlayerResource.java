...
public class PlayerResource {
  ...
  @Inject VerificationCodeRepository verificationCodeRepository;
  ...
  public List<jUserPlayer> listCompPlayer(
    ...
    if (competition == null) throw new BadRequestException("Invalid competition");
    ...
    Sex dbSex =
        switch (playerB ? competition.getPlayerBSex() : competition.getPlayerASex()) {
          case MALE -> Sex.MALE;
    ...
    return playerRepository
        .filter(search, dbSex, minAge, maxAge, admin, 0, 10)
        .map(jUserPlayer::new)
        .toList();
  }
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public jPage<List<jDirectorPlayer>> listPlayer(
      @QueryParam("search") String search,
      @DefaultValue("0") @QueryParam("page") int page,
      @DefaultValue("5") @QueryParam("pageSize") int pageSize) {
    if (pageSize <= 0) throw new BadRequestException("Page size has to be greater 0");
    return new jPage<>(
        playerRepository.countFilter(search, null, null, null, true),
        playerRepository
            .filter(search, null, null, null, true, page, pageSize)
            .map(jDirectorPlayer::new)
            .toList());
  }
  ...
  private void checkPlayerForm(jUserPlayerRegistrationForm form) {
    if (form.getFirstName() == null) throw new BadRequestException("First name may not be empty");
    if (form.getLastName() == null) throw new BadRequestException("Last name may not be empty");
    if (form.getBirthday() == null) throw new BadRequestException("Birthday may not be empty");
    if (form.getEmail() == null) throw new BadRequestException("E-Mail may not be empty");
    if (form.getPhone() == null) throw new BadRequestException("Phone may not be empty");
  }
  ...
  public String playerRegistration(jUserPlayerRegistrationForm playerForm) {
    Player exPlayer =
        playerRepository.getByName(playerForm.getFirstName(), playerForm.getLastName());
    if (exPlayer != null
        && (exPlayer.getVerificationCode() == null
            || (exPlayer.getVerificationCode() != null
                && exPlayer.getVerificationCode().getExpirationDate().isAfter(Instant.now()))))
      throw new WebApplicationException(
          "A player already exists with this name", Response.Status.CONFLICT);
    ...
    try {
      mailer.send(
          mailTemplates.verificationMail(
              newPlayer.getEmail(), verificationCode.getId().toString()));
    } catch (Exception e) {
  ...
  public String playerRegistration(jDirectorPlayerUpdateForm uPlayer) {
    ...
    if (player == null) throw new BadRequestException("Player does not exist");
  ...
  public String verification(@QueryParam("code") String code) {
    VerificationCode verificationCode =
        verificationCodeRepository.findByUUID(UUID.fromString(code));
  ...
  void clear() {
    List<VerificationCode> codes =
        verificationCodeRepository
            .find("FROM VerificationCode v WHERE v.expiration_date < " + "current_timestamp()")
            .list();
    for (var code : codes) {
      if (code.getPlayer() != null) playerRepository.delete(code.getPlayer());
