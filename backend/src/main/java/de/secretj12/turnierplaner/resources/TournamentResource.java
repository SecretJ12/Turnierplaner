...
public class TournamentResource {
  ...
  public List<jUserTournament> getAllTournaments() {
    if (securityIdentity.hasRole("director"))
      return tournaments
          ...
          .map(t -> (jUserTournament) t)
          .toList();
    else
      return tournaments.listAllVisible().stream()
  ...
  public jUserTournament getTournament(@PathParam("tourName") String name) {
    ...
    if (securityIdentity.hasRole("director")) return new jDirectorTournamentUpdate(tournament);
    else return new jUserTournament(tournament);
  }
  ...
  public String setCourts(@PathParam("tourName") String tourName, Set<jUserCourt> tCourts) {
    ...
    if (tournament == null) throw new NotFoundException("Could not find tournament");
  ...
  public String addTournament(jDirectorTournamentAdd tournament) {
    if (tournament.getName() == null) throw new BadRequestException("Tournament name is empty");
  ...
  public String updateTournament(jDirectorTournamentUpdate tournament) {
    ...
    if (possibleDuplicate != null && !possibleDuplicate.getId().equals(tournament.getId()))
      throw new WebApplicationException(
          "Tournament with this name already exist", Response.Status.CONFLICT);

    Tournament dbTournament = tournaments.findById(tournament.getId());
    if (dbTournament == null) throw new NotFoundException("Tournament with this id does not exist");
    checkDates(tournament);
