...
public class TestKnockoutTools {
  ...
  public void gen3Teams() {
    ...
    assertEquals(
        teams.get(0).getId(),
        newComp.getFinale().getDependentOn().getPreviousA().getTeamA().getId());

    assertEquals(
        teams.get(1).getId(),
        newComp.getFinale().getDependentOn().getPreviousB().getTeamA().getId());
    assertEquals(
        teams.get(2).getId(),
        newComp.getFinale().getDependentOn().getPreviousB().getTeamB().getId());

    assertNull(newComp.getThirdPlace());
    assertTrue(
        generated.stream().map(Match::getId).anyMatch(id -> newComp.getFinale().getId() == id));
  }
  ...
  public void gen4Teams() {
    ...
    assertEquals(
        teams.get(1).getId(),
        newComp.getFinale().getDependentOn().getPreviousA().getTeamB().getId());
    assertEquals(
        teams.get(2).getId(),
        newComp.getFinale().getDependentOn().getPreviousB().getTeamA().getId());
    assertEquals(
    ...
    assertEquals(
        newComp.getFinale().getDependentOn().getPreviousA().getId(),
        newComp.getThirdPlace().getDependentOn().getPreviousA().getId());
    assertEquals(
        newComp.getFinale().getDependentOn().getPreviousB().getId(),
